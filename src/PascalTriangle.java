...
public class PascalTriangle {
    ...
    public static ArrayList<ArrayList<Integer>>  writeRowToPascalTriangle(ArrayList<ArrayList<Integer>> someTriangle, int depth, int rowsAtPresent) {
        ...
        if (rowsAtPresent<depth) {
            if (rowsAtPresent ==1){ //special handling for second row
                newRow.add(1);
                newRow.add(1);
                ...
                newRow.add(1);//since last digit of triangle will always be 1
                someTriangle.add(newRow);
                rowsAtPresent = rowsAtPresent + 1;
                //recursively call the method till number of rows = desired depth
                writeRowToPascalTriangle(someTriangle, depth, rowsAtPresent);
            }
        }
        return someTriangle;
    ...
    // on Further thought I think recursive function can be shortned and add a special case
